...
; An INDEX is a 
(define-struct index (name documents lexicon positions) #:prefab #:mutable)
...
(define (number-of-documents index)
  (hash-count (index-documents index)))
...
; A LEXICON is a 
(define-struct lexicon (terms term-numbers) #:prefab #:mutable)
...
(define (make-empty-lexicon)
  (make-lexicon (make-hash)
                #f))

(define (lexicon-size lexicon)
  (hash-count (lexicon-terms lexicon)))
                 ...
                 (begin0
                   (sum-ec (:parallel 
                           ...
                           (begin 
                             ...
                             (when (zero? (remainder len MAX-RECORDS-IN-MEMORY))
                               (sort-and-save))
                             1))
                   (sort-and-save))))
             'replace)])
      ...
      (let-values ([(sorted-file temporary-file)
        ...
        (let* ([number-of-terms (lexicon-size lexicon)]
          ...
          (let ([term-numbers (make-vector (add1 (lexicon-size lexicon)) #"")])
            ...
            (hash-for-each (lexicon-terms lexicon)
                                 (λ (term term-number)
...
(define (calculate-term-frequencies lexicon file lexer)
  (let ([frequencies (make-hash)])
    (define (increase-frequency! term-number)
      (let ([f (hash-ref frequencies term-number
                               (λ ()
                                 (hash-set! frequencies term-number 0)
                                 0))])
        (hash-set! frequencies term-number (add1 f))))
