#lang racket/base
; ported to racket august 6th 2011 /soegaard
  ...
  (case-lambda
    ...
    [(in-bit-port)
     (let ([floor-log-n (sub1 (read-gamma in-bit-port))])
       (if (zero? floor-log-n)
           1 
           (let ([bs (read-bits floor-log-n in-bit-port)])
             (+ bs
                (arithmetic-shift 1 floor-log-n)))))]))
         ...
         [numbers-read
          (with-temporary-file file ()
            ...
            (with-input-from-bit-file file
              (λ ()
                (for/list ([_ (in-range (length numbers))])
                          (reader)))))])
